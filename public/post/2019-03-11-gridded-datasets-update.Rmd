...
* **488** of these are rasterized at around 0.1 decimal degrees (around 10 km)
* **40** datasets  are rasterized at >0.1 decimal degrees [example](https://www.gbif.org/dataset/81c2b870-f762-11e1-a439-00145eb45e9a)

These **final 40 datasets** with very large spacing are probably of most concern to those doing research projects with GBIF data. 
...
* WGS84 is assumed to calcuate distances. 
* **All uniuqe occurrence points** from each GBIF dataset have been used this time. 
...
This produces a simple measure of distance in decimal degrees that can be used for filtering out  datasets that might have a coordinate precision above what is expected from the study. The following api call will get all of the datasets flagged as "gridded" on the GBIF registry. 
...
This information can be incorporated easily into popular R packages for data cleaning, such as [CoordinateCleaner](https://github.com/ropensci/CoordinateCleaner). While this happens, I have created a simple function to extract rasterized keys from the registry. The following will install a package that can be used for extracting flagged datasets from the registry. 
