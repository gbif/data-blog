...
> In this blog post I will describe how I found each gridded dataset in GBIF. 
...
Throughout this post I have implicitly been assuming all coordinates are projected using WGS84, but this might not be the case specifically for 

Things that might still work but probably **will not** improve performance: 

* including more complex features
* including more nearest neighbor features
* including multiples of the most common distance

One could imagine looking for multiples of the most common distance as a way of detecting gridded datasets. However, this will likely not improve performance because it will only help identify datasets that are already very obviously gridded, and probably just add unnecessary complexity.
