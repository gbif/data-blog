...
Determining whether an occurrence record is suspicious can be somewhat subjective, and there is often a gray area between clearly legitimate and clearly problematic data. Common examples of suspicious records include mis-identifications where a species has been incorrectly identified, locality coordinate disagreements where the coordinates don't match the written description, outliers that fall far outside the expected range of a species, records from zoos or botanical gardens that represent captive rather than wild occurrences, and records that provide the location of the museum housing a specimen rather than where it was originally collected. Additionally, records placed at country centroids, records of terrestrial species appearing in the ocean, or coordinate problems such as swapped latitude and longitude values are relatively straightforward indicators of data quality issues.

While GBIF already implements some automated flagging of suspicious records through its data quality system, the rule-based annotation tool is designed to complement these existing checks by allowing users to identify patterns and issues that may be specific to certain taxa, regions, or datasets. The community-driven nature of rule-based annotations means that experts familiar with particular species or geographic areas can contribute their knowledge to help identify records that automated systems might miss, while still acknowledging that not all flagged records will be definitively wrong and some legitimate records may occasionally be caught by overly broad rules.
