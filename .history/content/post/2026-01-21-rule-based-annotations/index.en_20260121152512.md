...
Currently, rules are only available through the [labs.gbif.org/annotations](https://labs.gbif.org/annotations/#/) interface and the R package `gbifrules`. Rules **do not** appear on or get used on the main GBIF.org occurrence search and download systems. It is anticipated that if the tool becomes popular and widely used, that rules will be integrated into the main GBIF.org systems in the future. 
