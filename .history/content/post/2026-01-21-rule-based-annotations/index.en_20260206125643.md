...
Rules can look like range maps at first glance because they use polygons on a map. But **rules are not range maps**. They are more flexible and more powerful than traditional range maps because they are designed for **data cleaning**, not for defining a species’ full distribution.

Rules can **express uncertainty** in a way range maps often cannot. For example, a rule might say “records in this area are suspicious” rather than claiming a species is or is not present. Rules can also be **narrowly targeted** to specific datasets, time periods, or record attributes, which makes them useful for addressing known data issues without implying a definitive geographic range.

In short: range maps are about **where a species occurs**; rules are about **how to treat data**.
